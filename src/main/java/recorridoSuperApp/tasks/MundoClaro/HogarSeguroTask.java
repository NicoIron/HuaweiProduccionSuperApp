...
public class HogarSeguroTask implements Task {

	static String categoria = "Hogar Seguro";

	User user;

	public HogarSeguroTask(User user) {
		this.user = user;
	}

	Logger logger = Logger.getLogger(String.valueOf(HogarSeguroTask.class));

	@Override
	@Step("{0}  Navega por Hogar Seguro")
	public <T extends Actor> void performAs(T actor) {

		try {

			actor.attemptsTo(
					Click.on(BTN_HOGAR_SEGURO),
					WaitUntil.the(BTN_ENTENDIDO_MUNDOCLARO,
							isCurrentlyEnabled()).forNoMoreThan(30).seconds(),
					Click.on(BTN_ENTENDIDO_MUNDOCLARO),
					   WaitUntil.the(TXT_HOGAR_SEGURO,
						isCurrentlyEnabled()).forNoMoreThan(30).seconds(),
					Click.on(TXT_HOGAR_SEGURO));

			GoBack.DoWhileForBacKPage(actor, BTN_HOGAR_SEGURO,"Mundo Claro");
